...
(function () {
    ResizeGUI();
    d3.csv("../data/stockdata.csv", genType, function (data) {
        genRaw = data;
        mainjs();
    });
}());
...
function ResizeGUI() {
    $("#csbox").height($(document).height() - 50);
}
...
function displayCS() {
    var chart = cschart().Bheight(800);
    d3.select("#chart1").call(chart);
    var chart = barchart().mname("volume").margin(750).MValue("VOLUME");
